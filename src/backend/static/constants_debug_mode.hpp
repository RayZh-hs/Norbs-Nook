...
namespace norb::norbs_nook_constants {
    ...
    namespace log_constants {
        ...
        const inline std::string log_name                   = "generated/logs/norbs_nook_logging.log.right";
    }
    ...
    namespace validation_constants {
        ...
        const inline std::regex regex_valid_price              (R"(^\d{1,13}\.\d{1,13}$|^\d{1,13}$)");
        ...
        const inline std::regex regex_import                   (R"(^\ *import\ +(\d{1,10})\ +(\d{1,13}|[\d]{1,13}\.\d{1,13})\ *$)");
