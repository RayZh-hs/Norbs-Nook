...
namespace norb {
    ...
    const std::unordered_map<Logger::level, std::string> Logger::level_name = {
        {Logger::level::DEBUG, "DEBUG"}, {Logger::level::INFO, "INFO"}, {Logger::level::WARNING, "WARNING"}, {Logger::level::ERROR, "ERROR"}, {Logger::level::NONE, "NONE"},
    };
    ...
    void Logger::Log(const level level_, const std::string &msg) {
        assert(f_log.good());
